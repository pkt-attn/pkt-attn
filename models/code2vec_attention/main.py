...
def main(fold):
    ...
    parser.add_argument('--codevec_size', type=int, default=64)
    parser.add_argument('--np', type=int, default=200)  # <=200
    ...
    train_target_c, train_result, train_c_embed, train_x_result, train_c_list = data.load_data(train_path)
    print(val_path)
    val_p_id, val_c_id, val_source_token, val_path_token, val_target_token, val_context_mask,\
    val_target_c, val_result, val_c_embed, val_x_result, val_c_list = data.load_data(val_path)
    ...
    for idx in range(params.EPOCH):
        train_loss, train_accuracy, train_auc = train(model, params, optimizer, train_p_id, train_c_id,
                                    ...
                                    train_target_c, train_result, train_c_embed, train_x_result, train_c_list)
        print('Epoch %d/%d, loss : %3.5f, auc : %3.5f, accuracy : %3.5f' % (
            idx + 1, params.EPOCH, train_loss, train_auc, train_accuracy))

        valid_loss, valid_accuracy, valid_auc, _, _ = test(model, params, val_p_id, val_c_id, val_source_token,
                                                     val_path_token, val_target_token, val_context_mask,
                                                    val_target_c, val_result, val_c_embed, val_x_result, val_c_list)
    ...
    test_target_c, test_result, test_c_embed, test_x_result, test_c_list = data.load_data(test_path)

    test_loss, test_accuracy, test_auc, attn, concept = test(best_model, params, test_p_id, test_c_id, test_source_token,
                                              test_path_token, test_target_token, test_context_mask,
                                            test_target_c, test_result, test_c_embed, test_x_result, test_c_list)
    print('test auc : %3.5f, test accuracy : %3.5f' % (test_auc, test_accuracy))
    # np.savez('./attn_out.npz', attn=attn, concept=concept)
