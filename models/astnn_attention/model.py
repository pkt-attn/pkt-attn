...
class BatchTreeEncoder(nn.Module):
    ...
    def traverse_mul(self, node, batch_index):
        ...
        for i in range(size):
            ...
            temp = node[i][1: 3]  #限制个数，递归部分，限制子树个数   1: 3
            c_num = len(temp)
...
class MODEL(nn.Module):
    def __init__(self, num_concepts, num_problems, hidden_dim, hidden_layers,
        ...
        self.transdim = nn.Linear(self.ast_encode_dim, self.concept_embed_dim, bias=False)
    ...
    def attention(self, c_id, c_embed, gru_input, bs, seqlen):
        ...
        if(gru_input.shape[3] != self.concept_embed_dim):
    ...
    def attention_all(self, c_id, c_embed, gru_input, bs, seqlen):
